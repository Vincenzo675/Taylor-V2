...
const connectionOptions = {
  ...
  ...useQr && {
    printQRInTerminal: true
        ...
        logger,
        Browsers: ["Ubuntu", "Edge", "110.0.1587.56"],
        auth: {
  ...
  try {
    ...
    results.forEach(result => {
      if (result.status === "fulfilled") {
        console.info(`${result.value.message} completed successfully!`);
      }
    });
...
const successResults = _.map(
  _.filter(results, { status: "fulfilled" }),
  result => result.value.moduleName
);

const errorResults = _.map(
  _.filter(results, { status: "rejected" }),
  result => ({
...
global.plugins = _.chain(global.plugins)
  .toPairs()
  .sortBy(([a]) => a)
  .fromPairs()
  .value();
...
global.lib = _.chain(global.lib[""]?.lib)
  .toPairs()
  .sortBy(([a]) => a)
  .fromPairs()
  .value();
