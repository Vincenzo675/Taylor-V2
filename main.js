...
global.reloadHandler = async function reloadHandler(restatConn) {
  try {
    const Handler = await (await import(`./handler.js?updated=${Date.now()}`))?.catch(console.error);
  ...
  try {
    if (existsSync(dir)) {
      ...
      if (err) {
        ...
        const module = await await import(`${Helper.__filename(dir)}?updated=${Date.now()}`);
        plugins[filename] = module.default ? module.default : module;
      }
    } else {
